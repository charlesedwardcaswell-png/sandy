...
// ── SessionEndModal ───────────────────────────────────────────────────────────
export default function SessionEndModal({ session, characters, onConfirm, onClose }) {
    ...
    <div className="modal-overlay">
      <div className="modal" style={{ maxWidth: 440 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: '1rem' }}>
          <button className={`btn btn-sm ${tab === 'xp' ? 'btn-p' : ''}`} onClick={() => setTab('xp')}>XP Grant</button>
        ...
        {tab === 'xp' && (
          <>
            ...
            <div className="modal-section">
              ...
              <input
                type="text"
                placeholder={`e.g. Session ${sessionNum} — main story beat`}
            ...
            <div className="modal-section">
              ...
              {characters.length === 0 && (
                <div style={{ fontSize: 11, color: 'var(--text-muted)', fontStyle: 'italic' }}>No characters created.</div>
              )}
              {characters.map(c => (
                <label key={c.id} className="chk-row">
                  ...
                  />
                  {c.name} — {c.school} (currently {c.xp_total || 0} XP)
                </label>
              ))}
            </div>
          </>
        ...
        {tab === 'recap' && (
          <>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: '.75rem' }}>
              Saved to session archive. Fill in as much or as little as you like.
            </div>
            {[
        ...
        <div style={{ display: 'flex', gap: '.5rem', marginTop: '.5rem' }}>
          <button className="btn btn-p" onClick={handleConfirm}>
            {xpAmount > 0 ? `Grant ${xpAmount} XP & ` : ''}Archive Session {sessionNum}
          </button>
