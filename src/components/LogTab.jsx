    ...
    <div>
      ...
      {encounterLog && encounterLog.length > 0 && (
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', marginBottom: '.6rem' }}>
          ...
          {encounterLog.map((e, i) => (
            <div key={e.id || i} style={{ border: '1px solid var(--border)', borderRadius: 5, marginBottom: '.4rem', padding: '.55rem .75rem', background: 'var(--bg-panel)' }}>
              <div style={{ display: 'flex', gap: '.5rem', alignItems: 'center', marginBottom: '.25rem', flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--gold-dim)' }}>Session {e.session_number}</span>
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{formatDate(e.created_at)}</span>
                <span style={{ fontSize: 12, padding: '1px 6px', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text-muted)' }}>{e.setting}</span>
                <span style={{ fontSize: 12, padding: '1px 6px', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text-muted)' }}>{e.encounter_type}</span>
                <span style={{ fontSize: 12, color: 'var(--text-muted)', marginLeft: 'auto' }}>{e.rounds} rounds</span>
              </div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Party: {Array.isArray(e.party) ? e.party.map(p => p.name || p).join(', ') : (e.party || '—')}</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>vs: {Array.isArray(e.enemies) ? e.enemies.map(en => en.name || en).join(', ') : (e.enemies || '—')}</div>
            </div>
          ))}
        </div>
      )}
        ...
        sessionLog.map(s => {
          ...
          return (
            <div key={s.id} style={{ border: '1px solid var(--border)', borderRadius: 5, marginBottom: '.4rem', background: 'var(--bg-panel)', overflow: 'hidden' }}>
              ...
              >
                <i className="ti ti-calendar" style={{ fontSize: 14, color: 'var(--gold-dim)' }} />
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--gold-dim)' }}>Session {s.session_number}</span>
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{formatDate(s.closed_at || s.created_at)}</span>
                {sessionEncounters.length > 0 && (
                  <span style={{ fontSize: 11, padding: '1px 5px', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text-muted)' }}>
                ...
                {hasContent && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{exp ? '▲' : '▼'}</span>}
              </div>
                ...
                <div style={{ background: 'var(--bg-dark)', borderTop: '1px solid var(--border)' }}>
                  ...
                  {sessionEncounters.length > 0 && (
                    <div style={{ padding: '.5rem .75rem', borderBottom: s.events?.length > 0 ? '1px solid rgba(107,78,40,.3)' : 'none' }}>
                      <div style={{ fontSize: 11, color: 'var(--gold-dim)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '.4rem' }}>Encounters</div>
                      {sessionEncounters.map((e, i) => (
                        <div key={e.id || i} style={{ display: 'flex', gap: '.5rem', alignItems: 'flex-start', padding: '.35rem 0', borderBottom: i < sessionEncounters.length - 1 ? '1px solid rgba(107,78,40,.15)' : 'none' }}>
                          <i className="ti ti-swords" style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 1, flexShrink: 0 }} />
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', gap: '.4rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: 2 }}>
                              <span style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500 }}>{e.setting || 'Unknown location'}</span>
                              <span style={{ fontSize: 11, padding: '1px 5px', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text-muted)' }}>{e.encounter_type}</span>
                              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{e.rounds} rounds</span>
                              {e.difficulty && <span style={{ fontSize: 11, padding: '1px 5px', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--gold-dim)' }}>{e.difficulty}</span>}
                            </div>
                            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>vs {Array.isArray(e.enemies) ? e.enemies.map(en => en.name || en).join(', ') : (e.enemies || '—')}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {s.events?.length > 0 && (
                    <div style={{ padding: '.5rem .75rem' }}>
                      <div style={{ fontSize: 11, color: 'var(--gold-dim)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '.4rem' }}>
                        Session Events ({s.events.length})
                      </div>
                      <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                        {s.events.map((ev, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '.5rem', padding: '.2rem 0', borderBottom: '1px solid rgba(107,78,40,.1)', fontSize: 12 }}>
                            <i className={`ti ${ev.icon}`} style={{ fontSize: 12, color: 'var(--gold-dim)', flexShrink: 0, width: 14 }} />
