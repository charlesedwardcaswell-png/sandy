...
export default function LogTab({ encounterLog, sessionLog, skillLog }) {
  ...
  // skillLog shape: { skillName: { session: N, total: N } }
        ...
        sessionLog.map(s => {
          ...
          return (
            <div key={s.id} style={{ border: '1px solid var(--border)', borderRadius: 5, marginBottom: '.4rem', background: 'var(--bg-panel)', overflow: 'hidden' }}>
              <div
                style={{ padding: '.55rem .75rem', display: 'flex', gap: '.5rem', alignItems: 'center', cursor: hasRecap ? 'pointer' : 'default' }}
                onClick={() => hasRecap && setExpandedSession(e => ({ ...e, [s.id]: !e[s.id] }))}
              >
                ...
                {hasRecap && <span style={{ fontSize: 9, color: 'var(--text-muted)' }}>{exp ? '▲' : '▼'}</span>}
              </div>

              {exp && hasRecap && (
                <div style={{ padding: '.5rem .75rem', background: 'var(--bg-dark)', borderTop: '1px solid var(--border)' }}>
