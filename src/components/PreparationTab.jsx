...
// is created here, starting that session just flips visibility / hands items off. Phase 3: Shop
// Manager — shop-level management (create/rename/delete, hide-from-players); ShopTab keeps
// browsing/buying plus per-item editing (stock/randomize/quality/price/visibility/custom items).
export default function PreparationTab({ isDeveloper, characters, onUpdateCharacter, inventory, onUpdateInventory, onLogEvent, allSessions, quests, npcs, onSaveReveals, onCreatePrepSession, onSavePreparedEncounters, npcsFromLog, hideShopFromPlayers, onSetHideShopFromPlayers, onCreateQuest }) {
    ...
    <div>
      ...
      {subTab === 'shopmanager' && (
        <ShopManagerTab
          ...
          onSetHideShopFromPlayers={onSetHideShopFromPlayers}
        />
      )}
    </div>
  );
